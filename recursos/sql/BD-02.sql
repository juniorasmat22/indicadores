...
-- -----------------------------------------------------
-- Table `enfermeriaUNT`.`Curso`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `enfermeriaUNT`.`Curso` (
...
CREATE TABLE IF NOT EXISTS `enfermeriaUNT`.`fuente` (
  ...
  INDEX `fk_fuente_Curso1_idx` (`id_curso` ASC) ,
  CONSTRAINT `fk_fuente_Indicador1`
    FOREIGN KEY (`id_indicador`)
    REFERENCES `enfermeriaUNT`.`Indicador` (`id_indicador`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_fuente_Curso1`
    FOREIGN KEY (`id_curso`)
    REFERENCES `enfermeriaUNT`.`Curso` (`id_curso`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
